...
1. normal <esc-------i /> insert
2. normal <esc-------R /> replace
3. normal <esc-------V /> Visual
4. normal <esc-------S-V /> select-line
5. normal <esc-------C-V /> select-block
6. normal <esc-------: /> Command-line
	- q(uit) 退出，多窗口退出当前所在窗口
		- qa 关闭所有打开的窗口
	- w(rite) 写文件，相当于保存。
	- help + :w 在帮助手册中找到`:w`的含义
	- help + w 在帮助手册中找到`w`相关的操作，可用来学习
## 常见重新绑定
...
`r` means it will return to normal model when you finish one letter
`R` means it will return to insert model (you can write many words.)

 You use the `<ESC>` key a lot when using Vim: consider remapping Caps Lock to Escape ([macOS instructions](https://vim.fandom.com/wiki/Map_caps_lock_to_escape_in_macOS) [Windows instructions](https://kang000feng.github.io/blog/2015/02/04/remap-capslock-key/)).
...
`:help usr_08` It's about splitting windows.
...
- `:help {topic}` open help
    ...
    - `:help filename` opens help for the files in `help`
...
- Visual: `v`
- Visual Line: `V`
- Visual Block: `Ctrl-v` It seems like matrix
...
We are providing a well-documented basic config that you can use as a starting point. We recommend using this because it fixes some of Vim’s quirky default behavior. **Download our config [here](https://missing.csail.mit.edu/2020/files/vimrc) and save it to `~/.vimrc`.** Commands: `wget link`and`mv vimrc ~/.vimrc`
